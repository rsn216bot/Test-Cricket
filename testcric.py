...
with tab_matrix:
    ...
    if filt.empty or "lengthTypeId" not in filt.columns or "lineTypeId" not in filt.columns:
        ...
        if not mat.empty:
            ...
            # False Shot %: lower is better -> red to green with reversed map
            with t1:
                p1 = mat.pivot(index="lengthTypeId", columns="lineTypeId", values="False Shot %")
                styler = p1.style.background_gradient(cmap="RdYlGn_r", axis=None).format("{:.2f}")
                st.dataframe(styler, use_container_width=True, height=520)

            # Average: higher is better -> red to green normal map
            with t2:
                p2 = mat.pivot(index="lengthTypeId", columns="lineTypeId", values="Average")
                # style: treat inf nicely for display
                def fmt_avg(v):
                    if pd.isna(v):
                        return ""
                    if np.isinf(v):
                        return "∞"
                    return f"{v:.2f}"
                styler2 = p2.style.background_gradient(cmap="RdYlGn", axis=None).format(fmt_avg)
