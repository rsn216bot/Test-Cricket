...
def agg_block(df_in: pd.DataFrame, group_col: str) -> pd.DataFrame:
    ...
    # Drop empty/blank group labels
    g = g[g[group_col].notna()]
    g = g[g[group_col].astype(str).str.strip() != ""]
...
with tab_zones:
    ...
    if FIELD_POS_COL is None:
        ...
        fig, ax = plt.subplots(figsize=(5.5,5.5))  # smaller so it fits on screen
        draw_ground(ax)

        # OFF / LEG text (outside circle)
        ax.text(RADIUS * 1.04, 0, "OFF", ha="center", va="center", fontsize=10, weight="bold")
        ax.text(-RADIUS * 1.04, 0, "LEG", ha="center", va="center", fontsize=10, weight="bold")
        ...
        for ang in start_angles[:-1]:
            ...
            ax.plot([x0, x1], [y0, y1], linestyle="--", linewidth=0.5, alpha=0.45, color="gray")
        ...
        for i, label in enumerate(ZONE_LABELS):
            ...
            ax.text(x, y, str(runs), ha="center", va="center", fontsize=11, weight="bold", color="black")
...
with tab_matrix:
    ...
    if filt.empty or "lengthTypeId" not in filt.columns or "lineTypeId" not in filt.columns:
        ...
        if not mat.empty:
            ...
            # False Shot %: lower is better -> red to green (reversed map)
            with t1:
                p1 = mat.pivot(index="lengthTypeId", columns="lineTypeId", values="False Shot %")
                # Explicit relative scaling for full spectrum
                vmin = float(np.nanmin(p1.values.astype(float))) if p1.size else 0.0
                vmax = float(np.nanmax(p1.values.astype(float))) if p1.size else 1.0
                if vmin == vmax:
                    vmax = vmin + 1.0
                styler = (
                    p1.style
                    .background_gradient(cmap="RdYlGn_r", axis=None, vmin=vmin, vmax=vmax)
                    .format("{:.2f}")
                )
                st.dataframe(styler, use_container_width=True, height=520)

            # Average: higher is better -> red to green
            with t2:
                p2 = mat.pivot(index="lengthTypeId", columns="lineTypeId", values="Average")
                # For coloring, replace inf with finite max for gmap; keep display as ∞
                finite_vals = p2.replace([np.inf, -np.inf], np.nan).values.flatten()
                finite_vals = finite_vals[~np.isnan(finite_vals)]
                vmax = finite_vals.max() if finite_vals.size else 1.0
                color_map_source = p2.replace(np.inf, vmax)
                ...
                styler2 = (
                    p2.style
                    # Explicit relative scaling for Average (ignore inf for range, but color-map them at vmax)
                    finite_vals = p2.replace([np.inf, -np.inf], np.nan).values.astype(float)
                    vmin = float(np.nanmin(finite_vals)) if np.isfinite(finite_vals).any() else 0.0
                    vmax = float(np.nanmax(finite_vals)) if np.isfinite(finite_vals).any() else 1.0
                    if vmin == vmax:
                        vmax = vmin + 1.0
                    styler2_map = p2.replace(np.inf, vmax)
                    .style.background_gradient(cmap="RdYlGn", axis=None, vmin=vmin, vmax=vmax, gmap=styler2_map)
                    .format(fmt_avg)
                )
