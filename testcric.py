...
st.title("Batting Dashboard — Full Ground Wagon Wheel + Tables + Matrices")
st.caption("Use the top tabs to switch between Tables, Wagon Wheel, Zones, and Line–Length.")
...
# Top-level tabs
# =========================
tab_tables, tab_wheel, tab_zones, tab_matrix = st.tabs(["📋 Tables", "🎯 Wagon Wheel", "🧭 Zones", "🧮 Line–Length"])
...
# Common helpers for wheel & zones
RADIUS = 100
...
def polar_to_xy(angle_deg, r):
    theta = np.deg2rad(angle_deg)  # 0=keeper, 90=point
    x = r * np.sin(theta)
    y = -r * np.cos(theta)
    return x, y

# Run filter (used in wheel & zones)
...
runs_sel = st.sidebar.multiselect("Runs to show in wheel/zones", options=avail_runs, default=default_runs)
filt_runs = filt[filt.get("runsScored").isin(runs_sel)] if "runsScored" in filt.columns else filt

# ---------- WHEEL ----------
with tab_wheel:
    st.subheader("Wagon Wheel — Lines + 8 Zone Guides")
    if filt_runs.empty or "shotAngle" not in filt_runs.columns or "shotMagnitude" not in filt_runs.columns:
        ...
        if data.empty:
            ...
            base = (mag / mag.max() * (RADIUS * 0.95)) if mag.max() > 0 else np.full(len(mag), RADIUS * 0.6)
            runs = data["runsScored"].astype(int).values
            end_r = np.where(runs==6, RADIUS*1.15, np.where(runs==4, RADIUS*1.03, base))
    ...
    if FIELD_POS_COL is None:
        ...
        with sub1:
            ...
            for i, label in enumerate(ZONE_LABELS):
                ...
                wedge = Wedge((0,0), RADIUS, start, start+45, width=RADIUS*0.35, alpha=0.12)
                ax.add_patch(wedge)
                mid = start + 22.5
        ...
        with sub2:
            ...
            for i, label in enumerate(ZONE_LABELS):
                ...
                intensity = val / scale
                wedge = Wedge((0,0), RADIUS, start, start+45, width=RADIUS*0.9,
