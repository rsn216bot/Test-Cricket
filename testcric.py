...
with tab_zones:
    ...
    if FIELD_POS_COL is None:
        ...
        fig, ax = plt.subplots(figsize=(4,4))  # smaller so it fits on screen
        draw_ground(ax)
...
with tab_matrix:
    ...
    if filt.empty or "lengthTypeId" not in filt.columns or "lineTypeId" not in filt.columns:
        ...
        if not mat.empty:
            ...
            # False Shot %: lower is better -> green to red (relative scaling)
            with t1:
                
                # Pivot and enforce custom order
                ...
                # Percentile-based scaling for better contrast
                finite1 = p1.values.astype(float)
                vmin1 = float(np.nanpercentile(finite1, 5)) if np.isfinite(finite1).any() else 0.0
                vmax1 = float(np.nanpercentile(finite1, 95)) if np.isfinite(finite1).any() else 1.0
                if not np.isfinite(vmin1): vmin1 = 0.0
                if not np.isfinite(vmax1) or vmin1 == vmax1: vmax1 = vmin1 + 1.0

                styler1 = (
                    p1.style
                    .background_gradient(cmap="RdYlGn_r", axis=None, vmin=vmin1, vmax=vmax1)
                    .format("{:.2f}")
                )
                st.dataframe(styler1, use_container_width=True, height=520)

            # Average: higher is better -> green to red (relative scaling), ∞ handled
            with t2:
                ...
                # Compute percentiles on finite values for better contrast
                finite_vals = p2.replace([np.inf, -np.inf], np.nan).values.astype(float)
                if np.isfinite(finite_vals).any():
                    vmin2 = float(np.nanpercentile(finite_vals, 5))
                    vmax2 = float(np.nanpercentile(finite_vals, 95))
                    if not np.isfinite(vmin2): vmin2 = 0.0
                    if not np.isfinite(vmax2) or vmin2 == vmax2: vmax2 = vmin2 + 1.0
                else:
                    vmin2, vmax2 = 0.0, 1.0

                # For coloring, map inf to vmax
                gmap = p2.replace(np.inf, vmax2)
                ...
                styler2 = (
                    p2.style
                    .background_gradient(cmap="RdYlGn", axis=None, vmin=vmin2, vmax=vmax2, gmap=gmap)
                    .format(fmt_avg)
                )
