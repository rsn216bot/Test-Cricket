...
def load_data(url: str):
    try:
        ...
        st.error("Couldn't load from Google Sheets. Make sure the sheet is shared as 'Anyone with the link (Viewer)'.")
        st.stop()
    # Clean & enrich
...
# Batter selectbox with default "Virat Kohli"
batter_options = safe_unique("battingPlayer")
default_index = batter_options.index("Virat Kohli") if "Virat Kohli" in batter_options and len(batter_options)>0 else 0
batter = st.sidebar.selectbox("Batter", batter_options, index=default_index)
...
st.title("Batting Dashboard — Zones + Tables")
...
def agg_block(df_in: pd.DataFrame, group_col: str) -> pd.DataFrame:
    ...
    def safe_avg(r):
        d = r["Dismissals"] if pd.notna(r["Dismissals"]) else 0
        runs = r["Runs"] if pd.notna(r["Runs"]) else 0
        return "∞" if d == 0 else round(runs / d, 2)
...
with tab_tables:
    ...
    with bat_tabs[0]:
        st.dataframe(agg_block(filt, "battingFeetId"), use_container_width=True, height=360)
    with bat_tabs[1]:
        st.dataframe(agg_block(filt, "battingShotTypeId"), use_container_width=True, height=360)

    bowl_tabs = st.tabs(["Bowling Hand (vs batter)", "Bowling Detail (vs batter)", "Bowler (vs batter)"])
    with bowl_tabs[0]:
        st.dataframe(agg_block(filt, "bowlingHandId"), use_container_width=True, height=360)
    with bowl_tabs[1]:
        st.dataframe(agg_block(filt, "bowlingDetailId"), use_container_width=True, height=360)
    with bowl_tabs[2]:
        st.dataframe(agg_block(filt, "bowlerPlayer"), use_container_width=True, height=360)
...
with tab_zones:
    ...
    if FIELD_POS_COL is None:
        ...
        # Draw lavender wedges from center; dividers only outside pitch
        RADIUS = 100
        STRIP_H = 44
        STRIP_W = 10
        ...
        for i, label in enumerate(ZONE_LABELS):
            ...
            wedge = Wedge(center=(0,0), r=RADIUS, theta1=start, theta2=start+45,
                          width=RADIUS - r_inner, facecolor=LAVENDER, edgecolor="black", alpha=0.95)
            ax.add_patch(wedge)
            mid = start + 22.5
...
with tab_matrix:
    ...
    if filt.empty or "lengthTypeId" not in filt.columns or "lineTypeId" not in filt.columns:
        ...
        if not mat.empty:
            ...
            p1 = p1.round(2)
            st.write("**False Shot %**")
            st.dataframe(p1, use_container_width=True, height=500)

            # Average
            # Convert numeric averages to 2 decimals; keep "∞" as is
            def fmt_avg(x):
                try:
                    return f"{float(x):.2f}"
                except Exception:
                    return x
            avg_series = mat["Average"].apply(fmt_avg)
            mat2 = mat.copy()
            mat2["Average"] = avg_series
            p2 = mat2.pivot(index="lengthTypeId", columns="lineTypeId", values="Average")
            st.write("**Average**")
            st.dataframe(p2, use_container_width=True, height=500)
