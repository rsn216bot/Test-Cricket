...
# =========================
# Normalization for length/line labels (to avoid missing/None after ordering)
# =========================
def norm_length_label(s: str) -> str:
    if pd.isna(s):
        return ""
    t = str(s).strip().lower()
    # canonical map
    m = {
        ...
        "length": "Length",
        "good length": "Length",
        "back of a length": "Back of a length",
        "back-of-a-length": "Back of a length",
        "back of length": "Back of a length",
        "half tracker": "half tracker",
        "half-tracker": "half tracker",
        "short": "Short",
        "bouncer": "Short",
    }
    return m.get(t, str(s))

def norm_line_label(s: str) -> str:
    if pd.isna(s):
        return ""
    t = str(s).strip().lower()
    m = {
        "wide outside off stump": "wide outside off stump",
        "wide outside off": "wide outside off stump",
        "outside off stump": "outside off stump",
        "outside off": "outside off stump",
        "off stump": "off stump",
        "off": "off stump",
        "middle stump": "middle stump",
        "middle": "middle stump",
        "leg stump": "leg stump",
        "leg": "leg stump",
        "wide down leg": "wide down leg",
        "wide down leg side": "wide down leg",
    }
    return m.get(t, str(s))

# Create normalized columns
if "lengthTypeId" in filt.columns:
    filt["length_norm"] = filt["lengthTypeId"].apply(norm_length_label)
else:
    filt["length_norm"] = ""

if "lineTypeId" in filt.columns:
    filt["line_norm"] = filt["lineTypeId"].apply(norm_line_label)
else:
    filt["line_norm"] = ""

# Define desired orders and restrict to categories that actually exist
length_order_full = [
    "Full Toss",
    "Yorker",
    "Half volley",
...
length_cats = [x for x in length_order_full if x in set(filt["length_norm"].unique())]
line_cats = [x for x in line_order_full if x in set(filt["line_norm"].unique())]

filt["length_norm"] = pd.Categorical(filt["length_norm"], categories=length_cats, ordered=True)
filt["line_norm"] = pd.Categorical(filt["line_norm"], categories=line_cats, ordered=True)
...
with tab_zones:
    ...
    if FIELD_POS_COL is None:
        ...
        fig, ax = plt.subplots(figsize=(2.6,2.6))  # much smaller
        draw_ground(ax)
...
with tab_matrix:
    ...
    if filt.empty or "lengthTypeId" not in filt.columns or "lineTypeId" not in filt.columns:
        ...
        grp = filt.groupby(["length_norm", "line_norm"], dropna=False)
        rows = []
        for (length, line), d in grp:
            ...
            rows.append({"length": length, "line": line, "False Shot %": fs, "Average": avg})
        ...
        if not mat.empty:
            ...
            with t1:
                p1 = mat.pivot(index="length", columns="line", values="False Shot %").sort_index().sort_index(axis=1)
            ...
            with t2:
                p2 = mat.pivot(index="length", columns="line", values="Average").sort_index().sort_index(axis=1)
