...
st.title("Batting Dashboard — Zones-only View")
st.caption("Wagon wheel removed. One 'Shot Areas' view using 8 zones with purple intensity. Origin is striker's crease (one end of the pitch).")
...
# =========================
# Top-level tabs: Tables | Zones | Line–Length
# =========================
...
def polar_to_xy(angle_deg, r):
    theta = np.deg2rad(angle_deg)  # 0=keeper (down), 90=point (right)
    x = r * np.sin(theta)
    y = -r * np.cos(theta)
    return x, y

def ray_circle_intersection(p0, angle_deg, R):
    # Solve for point on circle (center 0,0, radius R) along ray from p0 at angle
    vx, vy = np.sin(np.deg2rad(angle_deg)), -np.cos(np.deg2rad(angle_deg))
    # Solve |p0 + t*v|^2 = R^2 -> t^2 + 2*(p0·v)*t + (|p0|^2 - R^2) = 0
    pv = p0[0]*vx + p0[1]*vy
    p2 = p0[0]**2 + p0[1]**2
    disc = pv*pv - (p2 - R*R)
    if disc < 0:
        return None
    t = -pv + np.sqrt(disc)  # positive intersection in ray direction
    return (p0[0] + t*vx, p0[1] + t*vy)

with tab_zones:
    # Run filter (purple zones reflect only selected runs)
    ...
    if FIELD_POS_COL is None:
        ...
        # Drawing params
        RADIUS = 100              # ground radius
        STRIP_H = 44              # pitch length (for drawing)
        STRIP_W = 10
        STRIKER = (0, -STRIP_H/2) # one end of pitch (keeper side), origin for zones
        PURPLE = (111/255, 66/255, 193/255)  # #6f42c1

        # 8 zone directions (start angles at 22.5 + 45*k)
        ...
        def draw_ground(ax):
            ...
            ax.plot([-STRIP_W/2, STRIP_W/2], [-STRIP_H/2, -STRIP_H/2], lw=0.8)
            # striker mark
            ax.plot([STRIKER[0]],[STRIKER[1]], marker="o", markersize=3, color="black")
            ax.set_aspect("equal")
            ax.set_xlim(-RADIUS*1.15, RADIUS*1.15)
            ax.set_ylim(-RADIUS*1.15, RADIUS*1.15)
            ax.axis("off")

        # Normalize intensities (purple alpha)
        max_runs = zone_runs["runsScored"].max()
        scale = max_runs if max_runs > 0 else 1

        st.subheader("Shot Areas (8 Zones, Purple Intensity)")
        ...
        # Build 8 triangular-sector polygons starting from STRIKER and clipped to circle
        # Use two boundary angles per zone: [start, start+45]
        for i, label in enumerate(ZONE_LABELS):
            start = start_angles[i]
            end = start + 45.0

            # Compute intersections of rays from STRIKER with field circle
            pA = ray_circle_intersection(STRIKER, start, RADIUS)
            pB = ray_circle_intersection(STRIKER, end, RADIUS)
            if pA is None or pB is None:
                continue

            # Arc between pA and pB about center (0,0)
            angA = np.degrees(np.arctan2(pA[1], pA[0])) % 360
            angB = np.degrees(np.arctan2(pB[1], pB[0])) % 360
            # choose direction (shorter arc)
            diff = (angB - angA) % 360
            if diff > 180:
                # go the other way
                arc_angles = np.linspace(angB, angA, 30)
                arc_pts = [(RADIUS*np.cos(np.deg2rad(a)), RADIUS*np.sin(np.deg2rad(a))) for a in arc_angles]
                path_pts = [STRIKER, pB] + arc_pts + [pA, STRIKER]
            else:
                arc_angles = np.linspace(angA, angB, 30)
                arc_pts = [(RADIUS*np.cos(np.deg2rad(a)), RADIUS*np.sin(np.deg2rad(a))) for a in arc_angles]
                path_pts = [STRIKER, pA] + arc_pts + [pB, STRIKER]

            # Build a Path and patch
            verts = [(x, y) for (x, y) in path_pts]
            codes = [Path.MOVETO] + [Path.LINETO]*(len(verts)-2) + [Path.CLOSEPOLY]
            path = Path(verts, codes)
            intensity = float(zone_runs.loc[zone_runs["zone"]==label, "runsScored"].values[0]) / scale if (zone_runs["zone"]==label).any() else 0.0
            patch = PathPatch(path, facecolor=PURPLE, alpha=float(intensity)*0.9 + 0.05, edgecolor="black", linewidth=0.6)
            ax.add_patch(patch)

            # Place runs text roughly at 60% of RADIUS along sector bisector from STRIKER
            mid = start + 22.5
            # Find point along ray from STRIKER
            vx, vy = np.sin(np.deg2rad(mid)), -np.cos(np.deg2rad(mid))
            label_pt = (STRIKER[0] + vx * (RADIUS*0.6), STRIKER[1] + vy * (RADIUS*0.6))
            runs = int(zone_runs.loc[zone_runs["zone"]==label, "runsScored"].values[0]) if (zone_runs["zone"]==label).any() else 0
            ax.text(label_pt[0], label_pt[1], str(runs), ha="center", va="center", fontsize=12, weight="bold", color="black")
