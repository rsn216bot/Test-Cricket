    ...
    if filt.empty or "lengthTypeId" not in filt.columns or "lineTypeId" not in filt.columns:
        ...
        if not mat.empty:
            ...
            with t1:
                ...
                vmin1 = float(np.nanmin(p1.values.astype(float))) if p1.size else 0.0
                vmax1 = float(np.nanmax(p1.values.astype(float))) if p1.size else 1.0
                if vmin1 == vmax1:
                    vmax1 = vmin1 + 1.0
                styler1 = (
                    p1.style
            ...
            with t2:
                ...
                # If all values are inf/NaN, default range
                if np.isfinite(finite_vals).any():
                    vmin2 = float(np.nanmin(finite_vals))
                    vmax2 = float(np.nanmax(finite_vals))
                    if vmin2 == vmax2:
                        vmax2 = vmin2 + 1.0
                else:
                    vmin2, vmax2 = 0.0, 1.0
                gmap = p2.replace(np.inf, vmax2)  # color-map inf as max
                # Formatter
                def fmt_avg(v):
                    if pd.isna(v):
                        return ""
